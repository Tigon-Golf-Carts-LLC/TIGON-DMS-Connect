# Yamaha - New Vehicle Manufacturer Mapping
...
> Only non-inherit values are Yamaha-specific overrides.
>
> **Important:** Yamaha does NOT have a template in `New_Cart_Converter.php`.
> All data comes from the DMS. Yamaha HAS dedicated color palette attributes.
>
> **Special Name Handling:**
> - Model alias: `Drive 2` -> `{MAKE} DRIVE2`
> - Model alias: `4L` -> `{MAKE} CROWN 4 LIFTED`
> - Model alias: `6L` -> `{MAKE} CROWN 6 LIFTED`
...
|---|---|
| `ID` | inherit |
| `post_title` | inherit (`YAMAHA(R) {MODEL} {Color} In {City} {State}`) |
...
| `_yoast_wpseo_primary_product_cat` | Term ID for `YAMAHA(R)` category |
...
| Meta Key | Value |
|---|---|
| `_yikes_woo_products_tabs` | **No Yamaha-specific warranty or spec tabs.** No tabs are injected for Yamaha models. |
...
| `_wcpa_product_meta` | Add-on list titled `YAMAHA(R) {Model} Add Ons` (follows global format) |
...
|---|---|---|
| `YAMAHA(R)` | Always | YES |
| `YAMAHA(R) {MODEL}` | If exists in system | YES (uses model aliases, see below) |
...
|---|---|
| `YAMAHA(R)` | Always |
| `YAMAHA(R) {MODEL}` | Always |
| `YAMAHA(R) {MODEL} {COLOR}` | Always |
...
**Yamaha has NO converter defaults.** All field values come directly from the DMS data:
