# Star EV - New Vehicle Manufacturer Mapping
...
> Only non-inherit values are Star EV-specific overrides.
>
> **Important:** Star EV does NOT have a template in `New_Cart_Converter.php`.
> All data comes from the DMS. Star EV HAS dedicated color palette attributes.
>
> **Special Name Handling:**
> - Internal make: `Star`
> - `make_with_symbol`: `STAR(R)`
> - Manufacturer taxonomy uses: `STAR EV(R)` (NOT `STAR(R)`)
> - Model prefix uses: `STAR EV(R)` (e.g. `STAR EV(R) Sirius`)
> - Attribute slug: `pa_star-ev-cart-colors` / `pa_star-ev-seat-colors`
...
|---|---|
| `ID` | inherit |
| `post_title` | inherit (`STAR(R) {MODEL} {Color} In {City} {State}`) |
...
| `_yoast_wpseo_primary_product_cat` | Term ID for `STAR EV(R)` category |
...
| Meta Key | Value |
|---|---|
| `_yikes_woo_products_tabs` | **No Star EV-specific warranty or spec tabs.** No tabs are injected for Star EV models. |
...
| `_wcpa_product_meta` | Add-on list titled `STAR(R) {Model} Add Ons` (follows global format) |
...
| Meta Key | Value |
|---|---|
| `_wc_facebook_enhanced_catalog_attributes_brand` | `STAR(R)` |
...
|---|---|---|
| `STAR EV(R)` | Always | YES (alias: Star -> `STAR EV(R)`) |
| `STAR EV(R) {MODEL}` | If exists in system | YES (model prefixed with `STAR EV(R)`) |
...
**Star EV has NO converter defaults.** All field values come directly from the DMS data:
