# Tara - New Vehicle Manufacturer Mapping
...
> Only non-inherit values are Tara-specific overrides.
>
> **Important:** Tara does NOT have a template in `New_Cart_Converter.php`.
> All data comes from the DMS. Tara does NOT have dedicated color palettes and falls back to generic color attributes.
...
|---|---|
| `ID` | inherit |
| `post_title` | inherit (`TARA(R) {MODEL} {Color} In {City} {State}`) |
...
| `_yoast_wpseo_primary_product_cat` | Term ID for `TARA(R)` category |
...
| Meta Key | Value |
|---|---|
| `_yikes_woo_products_tabs` | **No Tara-specific warranty or spec tabs.** No tabs are injected for Tara models. |
...
| `_wcpa_product_meta` | Add-on list titled `TARA(R) {Model} Add Ons` (follows global format) |
...
| `pa_lift-kit` | inherit | |
| `pa_location` | inherit | |
| `pa_cart-color` | `{cartColor}` | Generic fallback (no dedicated Tara palette) |
| `pa_seat-color` | `{seatColor}` | Generic fallback (no dedicated Tara palette) |
| `pa_sound-system` | `TARA(R) SOUND SYSTEM` | Brand-specific |
...
|---|---|---|
| `TARA(R)` | Always | YES |
| `TARA(R) {MODEL}` | If exists in system | YES |
...
| Manufacturer | `TARA(R)` |
| Model | `TARA(R) {MODEL}` |
| Sound System | `TARA(R) SOUND SYSTEM` |
...
**Tara has NO converter defaults.** All field values come directly from the DMS data:
