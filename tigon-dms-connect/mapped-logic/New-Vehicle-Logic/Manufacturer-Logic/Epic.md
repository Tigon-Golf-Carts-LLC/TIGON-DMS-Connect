# Epic - New Vehicle Manufacturer Mapping
...
> Only non-inherit values are Epic-specific overrides.
...
|---|---|
| `ID` | inherit |
| `post_title` | inherit (`EPIC(R) {MODEL} {Color} In {City} {State}`) |
...
| `_yoast_wpseo_metadesc` | inherit (uses `EPIC(R)` as make) |
...
| Meta Key | Value |
|---|---|
| `_yikes_woo_products_tabs` | **No Epic-specific warranty or spec tabs.** No tabs are injected for Epic models. |
...
| `_wcpa_product_meta` | Add-on list titled `EPIC(R) {Model} Add Ons` (e.g. `EPIC(R) E40L Add Ons`, `EPIC(R) E60 Add Ons`) |
...
|---|---|---|
| `pa_battery-type` | `AGM` | From shared default `battery.type = AGM` |
| `pa_battery-warranty` | `2` | From shared default `battery.warrantyLength = 2` |
| `pa_brush-guard` | **NO** | Epic override (Abstract_Cart.php:896) |
...
| `pa_vehicle-warranty` | `3` | From shared default `warrantyLength = 3` |
| `pa_year-of-vehicle` | inherit | |

### Available Epic Cart Colors

Black, Charcoal Gray, Dark Blue, Light Blue, Matte Black, Red Pearl, Silver, White Pearl

### Available Epic Seat Colors

Black (shared default for all Epic models)

---

## term_relationships - Categories

All Epic models receive these categories (based on shared defaults: `isElectric=true`, `battery.type=AGM`, `battery.packVoltage=36`, `isStreetLegal=true`):

| Category | Condition | Applied? |
|---|---|---|
| `EPIC(R)` | Always | YES |
| `EPIC(R) {MODEL}` | If exists in system | YES |
...
| `LITHIUM` | `battery.type = Lithium` | **NO** (AGM) |
| `LEAD-ACID` | `battery.type = Lead` | **NO** (AGM) |
| `36 VOLT` | Electric, `packVoltage=36` | YES |
...
> **Note:** Battery type is AGM. The category mapping checks for "Lithium" and "Lead" only, so AGM does NOT match either battery-type category.
...
| `EPIC(R)` | Always |
| `EPIC(R) {MODEL}` | Always |
| `EPIC(R) {MODEL} {COLOR}` | Always |
| All other tags | inherit from global |

---

## Shared Defaults from Converter (All Epic Models)

These values are set in `New_Cart_Converter.php` for every Epic model and feed into the mappings above:
