# Tomberlin - New Vehicle Manufacturer Mapping
...
> Only non-inherit values are Tomberlin-specific overrides.
>
> **Important:** Tomberlin does NOT have a template in `New_Cart_Converter.php`.
> All data comes from the DMS. Tomberlin HAS dedicated color palette attributes.
...
|---|---|
| `ID` | inherit |
| `post_title` | inherit (`TOMBERLIN(R) {MODEL} {Color} In {City} {State}`) |
...
| `_yoast_wpseo_primary_product_cat` | Term ID for `TOMBERLIN(R)` category |
...
| Meta Key | Value |
|---|---|
| `_yikes_woo_products_tabs` | **No Tomberlin-specific warranty or spec tabs.** No tabs are injected for Tomberlin models. |
...
| `_wcpa_product_meta` | Add-on list titled `TOMBERLIN(R) {Model} Add Ons` (follows global format) |
...
| `pa_lift-kit` | inherit | |
| `pa_location` | inherit | |
| `pa_tomberlin-cart-colors` | `{cartColor}` from Tomberlin palette | Tomberlin-specific attribute slug |
| `pa_tomberlin-seat-colors` | `{seatColor}` from Tomberlin palette | Tomberlin-specific attribute slug |
| `pa_sound-system` | `TOMBERLIN(R) SOUND SYSTEM` | Brand-specific |
...
|---|---|---|
| `TOMBERLIN(R)` | Always | YES |
| `TOMBERLIN(R) {MODEL}` | If exists in system | YES |
...
| Manufacturer | `TOMBERLIN(R)` |
| Model | `TOMBERLIN(R) {MODEL}` |
| Sound System | `TOMBERLIN(R) SOUND SYSTEM` |
...
|---|---|
| `TOMBERLIN(R)` | Always |
| `TOMBERLIN(R) {MODEL}` | Always |
| `TOMBERLIN(R) {MODEL} {COLOR}` | Always |
...
**Tomberlin has NO converter defaults.** All field values come directly from the DMS data:
