# Atlas - New Vehicle Manufacturer Mapping
...
> Only non-inherit values are Atlas-specific overrides.
>
> **Important:** Atlas does NOT have a template in `New_Cart_Converter.php`.
> All data comes from the DMS. Atlas does NOT have dedicated color palettes and falls back to generic color attributes.
> Atlas is not currently referenced in the codebase and may be a future brand addition.
...
|---|---|
| `ID` | inherit |
| `post_title` | inherit (`ATLAS(R) {MODEL} {Color} In {City} {State}`) |
...
| `_yoast_wpseo_primary_product_cat` | Term ID for `ATLAS(R)` category |
...
| Meta Key | Value |
|---|---|
| `_yikes_woo_products_tabs` | **No Atlas-specific warranty or spec tabs.** No tabs are injected for Atlas models. |
...
| `_wcpa_product_meta` | Add-on list titled `ATLAS(R) {Model} Add Ons` (follows global format) |
...
|---|---|---|
| `ATLAS(R)` | Always | YES |
| `ATLAS(R) {MODEL}` | If exists in system | YES |
...
|---|---|
| `ATLAS(R)` | Always |
| `ATLAS(R) {MODEL}` | Always |
| `ATLAS(R) {MODEL} {COLOR}` | Always |
...
**Atlas has NO converter defaults.** All field values come directly from the DMS data:
