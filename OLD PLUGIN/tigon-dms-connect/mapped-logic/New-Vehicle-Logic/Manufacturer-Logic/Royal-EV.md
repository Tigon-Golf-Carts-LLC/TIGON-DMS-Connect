# Royal EV - New Vehicle Manufacturer Mapping
...
> Only non-inherit values are Royal EV-specific overrides.
>
> **Important:** Royal EV does NOT have a template in `New_Cart_Converter.php`.
> All data comes from the DMS. Royal EV HAS dedicated color palette attributes.
...
|---|---|
| `ID` | inherit |
| `post_title` | inherit (`ROYAL EV(R) {MODEL} {Color} In {City} {State}`) |
...
| `_yoast_wpseo_primary_product_cat` | Term ID for `ROYAL EV(R)` category |
...
| Meta Key | Value |
|---|---|
| `_yikes_woo_products_tabs` | **No Royal EV-specific warranty or spec tabs.** No tabs are injected for Royal EV models. |
...
| `_wcpa_product_meta` | Add-on list titled `ROYAL EV(R) {Model} Add Ons` (follows global format) |
...
| `pa_lift-kit` | inherit | |
| `pa_location` | inherit | |
| `pa_royal-ev-cart-colors` | `{cartColor}` from Royal EV palette | Royal EV-specific attribute slug |
| `pa_royal-ev-seat-colors` | `{seatColor}` from Royal EV palette | Royal EV-specific attribute slug |
| `pa_sound-system` | `ROYAL EV(R) SOUND SYSTEM` | Brand-specific |
...
|---|---|---|
| `ROYAL EV(R)` | Always | YES |
| `ROYAL EV(R) {MODEL}` | If exists in system | YES |
...
| Manufacturer | `ROYAL EV(R)` |
| Model | `ROYAL EV(R) {MODEL}` |
| Sound System | `ROYAL EV(R) SOUND SYSTEM` |
...
|---|---|
| `ROYAL EV(R)` | Always |
| `ROYAL EV(R) {MODEL}` | Always |
| `ROYAL EV(R) {MODEL} {COLOR}` | Always |
...
**Royal EV has NO converter defaults.** All field values come directly from the DMS data:
