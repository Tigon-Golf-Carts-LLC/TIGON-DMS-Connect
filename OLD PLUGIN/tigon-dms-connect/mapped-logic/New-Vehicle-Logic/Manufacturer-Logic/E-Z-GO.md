# E-Z-GO - New Vehicle Manufacturer Mapping
...
> Only non-inherit values are E-Z-GO-specific overrides.
>
> **Important:** E-Z-GO does NOT have a template in `New_Cart_Converter.php`.
> All data comes from the DMS. E-Z-GO HAS dedicated color palette attributes.
>
> **Special Name Handling:**
> - Internal make key: `ezgo`
> - `make_with_symbol`: `EZGO(R)`
> - Category/model prefix uses: `EZ-GO(R)` (NOT `EZGO(R)`)
> - Tag prefix uses: `EZGO(R)`
> - Attribute slug: `pa_ezgo-cart-colors` / `pa_ezgo-seat-colors`
...
|---|---|
| `ID` | inherit |
| `post_title` | inherit (`EZGO(R) {MODEL} {Color} In {City} {State}`) |
...
| `_yoast_wpseo_primary_product_cat` | Term ID for `EZ-GO(R)` category |
...
| Meta Key | Value |
|---|---|
| `_yikes_woo_products_tabs` | **No E-Z-GO-specific warranty or spec tabs.** No tabs are injected for E-Z-GO models. |
...
| `_wcpa_product_meta` | Add-on list titled `EZGO(R) {Model} Add Ons` (follows global format) |
...
| `pa_lift-kit` | inherit | |
| `pa_location` | inherit | |
| `pa_ezgo-cart-colors` | `{cartColor}` from EZGO palette | E-Z-GO-specific attribute slug (uses `ezgo`, not `ez-go`) |
| `pa_ezgo-seat-colors` | `{seatColor}` from EZGO palette | E-Z-GO-specific attribute slug (uses `ezgo`, not `ez-go`) |
| `pa_sound-system` | `EZGO(R) SOUND SYSTEM` | Brand-specific |
...
> **Important:** Categories use `EZ-GO(R)` prefix (NOT `EZGO(R)`):

| Category | Condition | Applied? |
|---|---|---|
| `EZ-GO(R)` | Always | YES (alias: EZGO -> `EZ-GO(R)`) |
| `EZ-GO(R) {MODEL}` | If exists in system | YES (model prefixed with `EZ-GO(R)`) |
...
| Manufacturer | `EZGO(R)` |
| Model | `EZ-GO(R) {MODEL}` (uses `EZ-GO(R)` prefix, not `EZGO(R)`) |
| Sound System | `EZGO(R) SOUND SYSTEM` |
...
**E-Z-GO has NO converter defaults.** All field values come directly from the DMS data:
