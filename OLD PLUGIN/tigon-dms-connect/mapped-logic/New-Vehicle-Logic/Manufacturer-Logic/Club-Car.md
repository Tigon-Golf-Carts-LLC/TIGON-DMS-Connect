# Club Car - New Vehicle Manufacturer Mapping
...
> Only non-inherit values are Club Car-specific overrides.
>
> **Important:** Club Car does NOT have a template in `New_Cart_Converter.php`.
> All data comes from the DMS. Club Car HAS dedicated color palette attributes.
>
> **Special Name Handling:**
> - Model alias: `DS` -> `{MAKE} DS ELECTRIC`
> - Model alias: `Precedent` -> `{MAKE} PRECEDENT ELECTRIC`
...
|---|---|
| `ID` | inherit |
| `post_title` | inherit (`CLUB CAR(R) {MODEL} {Color} In {City} {State}`) |
...
| `_yoast_wpseo_primary_product_cat` | Term ID for `CLUB CAR(R)` category |
...
| Meta Key | Value |
|---|---|
| `_yikes_woo_products_tabs` | **No Club Car-specific warranty or spec tabs.** No tabs are injected for Club Car models. |
...
| `_wcpa_product_meta` | Add-on list titled `CLUB CAR(R) {Model} Add Ons` (follows global format) |
...
|---|---|---|
| `CLUB CAR(R)` | Always | YES |
| `CLUB CAR(R) {MODEL}` | If exists in system | YES (uses model aliases, see below) |
...
|---|---|
| `CLUB CAR(R)` | Always |
| `CLUB CAR(R) {MODEL}` | Always |
| `CLUB CAR(R) {MODEL} {COLOR}` | Always |
...
**Club Car has NO converter defaults.** All field values come directly from the DMS data:
