# TEKO EV - New Vehicle Manufacturer Mapping
...
> Only non-inherit values are TEKO EV-specific overrides.
>
> **Important:** TEKO EV does NOT have a template in `New_Cart_Converter.php`.
> All data comes from the DMS. TEKO EV does NOT have dedicated color palettes and falls back to generic color attributes.
...
|---|---|
| `ID` | inherit |
| `post_title` | inherit (`TEKO EV(R) {MODEL} {Color} In {City} {State}`) |
...
| `_yoast_wpseo_primary_product_cat` | Term ID for `TEKO EV(R)` category |
...
| Meta Key | Value |
|---|---|
| `_yikes_woo_products_tabs` | **No TEKO EV-specific warranty or spec tabs.** No tabs are injected for TEKO EV models. |
...
| `_wcpa_product_meta` | Add-on list titled `TEKO EV(R) {Model} Add Ons` (follows global format) |
...
|---|---|---|
| `TEKO EV(R)` | Always | YES |
| `TEKO EV(R) {MODEL}` | If exists in system | YES |
...
|---|---|
| `TEKO EV(R)` | Always |
| `TEKO EV(R) {MODEL}` | Always |
| `TEKO EV(R) {MODEL} {COLOR}` | Always |
...
**TEKO EV has NO converter defaults.** All field values come directly from the DMS data:
